...
Before running vector search queries, create a vector search index on the field that contains your embeddings. Use the `createSearchIndex()` method to create a vector search index.
...
1. Insert sample data.
    ...
    | ----- | ---- | -------- | ----------- |
    | `documents` | array of documents | Yes | The documents to insert into the collection. |
    | `ordered` | boolean | Optional | If `Yes` (the default), MongoDB inserts documents in order and stops after the first error. If `No`, MongoDB continues inserting the rest after an error. |
    ...
    !!! note
        The example embeddings are shortened for readability. In practice, embedding models typically generate vectors with hundreds or thousands of dimensions.
...
[Update a search index :material-arrow-right:](update-vector-search-index.md){.md-button}
