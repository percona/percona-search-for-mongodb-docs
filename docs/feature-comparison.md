...
| Automatic embeddings | Yes | Voyage AI | Voyage AI | Voyage AI |
...
!!! note
    - Percona Search for MongoDB supports automatic embeddings with Voyage AI and OpenAI-compatible providers.
    - Search indexes are created and managed using `createSearchIndex()`, `updateSearchIndex()`, and `dropSearchIndex()`.
    - For indexes that do not use `autoEmbed`, vector search requires embeddings generated by your application before indexing.
